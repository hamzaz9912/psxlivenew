...
def display_comprehensive_intraday_forecasts():
    ...
    with tab1:
        ...
        if historical_kse is not None and not historical_kse.empty:
            # Use live_price if available, otherwise use the most recent historical price
            if live_price and live_price > 100000:  # Sanity check - KSE-100 should be > 100k
                current_price = live_price
            else:
                # Use the most recent closing price from historical data
                if 'close' in historical_kse.columns:
                    historical_close = historical_kse['close'].iloc[-1]
                    # If historical price is very old/low, try to get a better estimate
                    if historical_close < 100000:
                        # Try to get index value as fallback
                        if hasattr(st.session_state, 'enhanced_psx_fetcher'):
                            try:
                                kse_index = st.session_state.enhanced_psx_fetcher.get_kse100_index_value()
                                if kse_index and kse_index.get('value'):
                                    current_price = kse_index['value']
                                    st.info(f"Using KSE-100 Index: PKR {current_price:,.2f}")
                                else:
                                    current_price = 152700  # Default to current market level
                            except:
                                current_price = 152700
                        else:
                            current_price = 152700
                    else:
        ...
        if current_price and current_price > 100000:
            ...
            if not trading_day or forecast_type == "next_day" or forecast_type == "closing_session":
                ...
                # Make it lighter so forecast is more visible
                if live_price and live_price > 0:
                    fig.add_hline(
                        ...
                        line_width=1,
                        annotation_text=f"Today: {live_price:,.0f}",
                ...
                if afternoon_session is not None and not afternoon_session.empty:
                    ...
                    with col1:
                        if next_day_forecast is not None and not next_day_forecast.empty:
                            forecast_start = next_day_forecast['predicted_price'].iloc[0]
                            st.metric("Forecast Open (09:30)", f"PKR {forecast_start:,.2f}")
                        else:
